...
class CAGService:
    ...
    def generate(
        ...
        source_file = crag_result.get("source_file", [])
        ...
        if use_cache and answer and _confident:
            ...
            self.cache.set(query, {"answer": cached_answer, "source_file": source_file})
        ...
        for item in queries:
            if isinstance(item, dict) and "query" in item and "answer" in item:
                ...
                if query not in self.cache:
                    self.cache.set(query, {"answer": item["answer"], "source_file": []})
                    cached_count += 1
